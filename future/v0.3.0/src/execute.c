...
// #s — platform-specific syscall; result stored in g[0]
void exec_syscall(lo3_val a1, lo3_val a2) {

	if (a1.chooseType || a2.chooseType) {
		lo3_error("Both args for #s must be numeric (syscall number and argument)", "");
		return;
	}
	...
	g_set(0, result);
#else
	lo3_warn("syscall (#s) is only supported on Linux", "");
#endif
}
